                ...
                @if($user->currentSubscriptionPlan())
                    ...
                    <div class="alert alert-warning" role="alert">
                        ...
                        @if($user->currentSubscriptionPlan()->ends_at->isFuture())
                            <p>Hi, {{ $user->first_name }}. Your subscription <b>limit for selling your car is over. Kindly pick a plan</b></p>
                        @else
                            <p>Hi, {{ $user->first_name }}. Your subscription plan ended on <b>{{ $user->currentSubscriptionPlan()->ends_at }}. Kindly pick a plan</b></p>
                        @endif
                    </div>
                    @elseif($user->subscriptionPlanLimit() && $user->subscriptionPlanLimit() > 0)
                        <div class="alert alert-success" role="alert">
                            ...
                            <p>Hi, {{ $user->first_name }}. You already have a active plan which currently have <b>{{ $user->subscriptionPlanLimit() }} cars adding limit</b></p>
                        </div>
                    @endif
                @endif
            @endif
                    ...
                    @if($user)
                        @if($user->currentSubscriptionPlan())
                            ...
                            @if($user->subscriptionPlanLimit() && $user->subscriptionPlanLimit() < 1)
                                <a href="{{ route('seller.pages.paymentMethods', ['slug' => $plan->slug]) }}" class="pick-a-plan pap-abs">Pick a Plan</a>
                            @endif
                            @if(!$user->currentSubscriptionPlan()->ends_at->isFuture() || $user->subscriptionPlanLimit() < 1)
                                <a href="{{ route('seller.pages.paymentMethods', ['slug' => $plan->slug]) }}" class="pick-a-plan pap-abs">Pick a Plan</a>
                            @endif
