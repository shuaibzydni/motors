...
use App\Http\Requests\Buyers\RegisterRequest;
use App\Http\Requests\Buyers\VerifyPasswordRequest;
use App\Http\Requests\SocialLoginRequest;
use App\Mail\ForgotPasswordOtpMail;
use App\Models\Buyer;
use Facades\App\Services\BuyerAuthService;
...
class AuthController extends Controller
{
    ...
    public function login(LoginRequest $request)
    {
        ...
        $authData = BuyerAuthService::login($credentials, $request);
    ...
    public function socialLogin(SocialLoginRequest $request)
    {
        ...
        $user = Buyer::where('email', $request->email)->firstOrFail();
    ...
    public function forgotPassword(Request $request)
    {
        ...
        $buyer = Buyer::where('email', $request->email)->firstOrFail();

        $otp = rand(1000,9999);
        Log::info("otp = ".$otp);
        $buyer->update(['otp' => $otp]);

        Mail::to($buyer)->send(new ForgotPasswordOtpMail($otp));
